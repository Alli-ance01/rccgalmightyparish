...
export default function EventDetail() {
  ...
  return <PublicLayout><section className="container py-12 sm:py-20"><Link href="/events" className="inline-flex items-center gap-2 text-xs font-extrabold text-[#0b4ab8]"><ArrowLeft className="h-3.5 w-3.5" />All events</Link>{event.coverImageUrl && <img src={event.coverImageUrl} alt="" className="mt-8 aspect-[2.2/1] w-full rounded-[1.5rem] object-cover" />}<div className="mt-10 grid gap-10 lg:grid-cols-[1.2fr_0.8fr]"><div><p className="eyebrow text-[#0b4ab8]">TAP gathering</p><h1 className="display mt-5 text-6xl leading-[0.9] text-[#10213e]">{event.title}</h1><p className="mt-6 text-base leading-8 text-slate-600">{event.description}</p></div><aside className="rounded-[1.4rem] bg-[#eaf2ff] p-7"><p className="eyebrow text-[#0b4ab8]">Event details</p><p className="mt-7 flex gap-3 text-sm font-bold text-slate-700"><CalendarDays className="h-4 w-4 shrink-0 text-[#0b4ab8]" />{new Date(event.startsAt).toLocaleString("en-NG", { dateStyle: "full", timeStyle: "short" })}</p><p className="mt-5 flex gap-3 text-sm font-bold text-slate-700"><MapPin className="h-4 w-4 shrink-0 text-[#0b4ab8]" />{event.location}</p>{event.registrationUrl && <a href={event.registrationUrl} target="_blank" rel="noreferrer" className="tap-button mt-8 inline-flex items-center gap-2 rounded-full bg-[#0b4ab8] px-4 py-2.5 text-xs font-extrabold text-white">Register now<ExternalLink className="h-3.5 w-3.5" /></a>}</aside></div></section></PublicLayout>;
}
