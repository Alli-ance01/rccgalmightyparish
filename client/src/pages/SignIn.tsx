    ...
    <PublicLayout>
      <section className="container grid min-h-[calc(100vh-12rem)] items-center py-14 sm:py-20">
        <div className="mx-auto w-full max-w-xl">
          ...
          <p className="mt-5 max-w-lg text-sm leading-7 text-slate-600">This private access page is for approved web workers and the Master Admin. Worker requests are reviewed before access is granted.</p>

          <div className="mt-8 grid grid-cols-2 gap-2 rounded-2xl bg-slate-100 p-1.5">
            {(["sign-in", "staff"] as Mode[]).map(item => (
              <button key={item} type="button" onClick={() => selectMode(item)} className={`tap-button rounded-xl px-3 py-2.5 text-xs font-extrabold ${mode === item ? "bg-white text-[#0b4ab8] shadow-sm" : "text-slate-500"}`}>
                {item === "sign-in" ? "Sign in" : "Request worker access"}
              </button>
            ))}
          </div>
          ...
          <div className="mt-5 rounded-[1.5rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            {mode === "sign-in" ? (
              ...
              <form onSubmit={submitRegister} className="grid gap-5">
                <FormTitle icon={UsersRound} title="Request worker access" copy="For web administrators and media workers. Your request stays pending until the Master Admin approves access." />
                ...
                <Button type="submit" disabled={register.isPending} className="mt-1 h-11 rounded-xl bg-[#0b4ab8] font-extrabold">{register.isPending ? "Submitting…" : "Send worker access request"}</Button>
