...
describe("application route destinations", () => {
  it("keeps all primary visitor journeys and staff entry points registered", () => {
    ...
    ["/", "/about", "/ministries", "/junior-church", "/sermons", "/events", "/announcements", "/media", "/news", "/visit", "/contact", "/leadership", "/give", "/sign-in", "/account", "/admin", "/admin/approvals"].forEach(path => expect(app).toContain(`path=\"${path}`));
  });
  ...
  it("uses an intentional staff sign-in destination and exposes account and announcement routes from shared navigation", () => {
    ...
    expect(header).toContain('{ label: "Account", href: "/account" }');
